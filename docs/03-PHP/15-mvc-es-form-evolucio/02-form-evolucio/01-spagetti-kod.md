...
slug: /php/mvc-es-form-evolucio/form-evolucio/spagetti-kod
...
- `name="add-subject"` → ez jelzi, hogy a felhasználó a hozzáadás gombot nyomta meg
...
```php
<a href="index.php?edit-subject=<?= $subject['id'] ?>">Módosítás</a>
```

Ez a link egy `edit-subject` nevű GET paramétert ad át, például:

```
index.php?edit-subject=3
```
...
```php
if (isset($_GET['edit-subject'])) {
    $id = $_GET['edit-subject'];
...
- Megnézi, hogy kaptunk‑e `edit-subject` paramétert.
